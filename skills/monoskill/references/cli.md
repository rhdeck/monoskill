...
The skill and CLI install separately. On a clean machine, use the canonical GitHub-backed invocation shown below; it downloads and runs the real CLI without a prior global install. If `monoskill` is already installed, `monoskill <command>` is equivalent. Run `npx --yes github:rhdeck/monoskill --help` before acting if the available version may differ from this reference.
...
```bash
npx --yes github:rhdeck/monoskill add <source> --name <name> --dry-run --json
```

Install into the current project after inspecting the preview:

```bash
npx --yes github:rhdeck/monoskill add <source> --name <name>
npx --yes github:rhdeck/monoskill add <source> --name <name> --agent codex --agent claude-code
```
...
```bash
npx --yes github:rhdeck/monoskill add <source> --name <name> --agent codex --agent claude-code --global --yes --json
```
...
```bash
npx --yes github:rhdeck/monoskill build <source> --name <name> --output <dir>
```
...
```bash
npx --yes github:rhdeck/monoskill build <source> --name <name> --archive --output <file.skill>
```
...
```bash
npx --yes github:rhdeck/monoskill package <skill-dir> --output <file.skill>
```
...
```bash
npx --yes github:rhdeck/monoskill check <skill-dir>
npx --yes github:rhdeck/monoskill check <skill-dir> --json
```
...
```bash
npx --yes github:rhdeck/monoskill update <skill-dir>
```
