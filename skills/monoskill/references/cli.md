...
The skill and CLI install separately. On a clean machine, use the pinned GitHub-backed invocation shown below; it downloads and runs the matching 0.3.0 CLI without a prior global install. If `monoskill` 0.3.0 is already installed, `monoskill <command>` is equivalent. Run `npx --yes github:rhdeck/monoskill#82fff64 --help` before acting.
...
```bash
npx --yes github:rhdeck/monoskill#82fff64 add <source> --name <name> --dry-run --json
```

Install into the current project after inspecting the preview:

```bash
npx --yes github:rhdeck/monoskill#82fff64 add <source> --name <name>
npx --yes github:rhdeck/monoskill#82fff64 add <source> --name <name> --agent codex --agent claude-code
```
...
```bash
npx --yes github:rhdeck/monoskill#82fff64 add <source> --name <name> --agent codex --agent claude-code --global --yes --json
```
...
```bash
npx --yes github:rhdeck/monoskill#82fff64 build <source> --name <name> --output <dir>
```
...
```bash
npx --yes github:rhdeck/monoskill#82fff64 build <source> --name <name> --archive --output <file.skill>
```
...
```bash
npx --yes github:rhdeck/monoskill#82fff64 package <skill-dir> --output <file.skill>
```
...
```bash
npx --yes github:rhdeck/monoskill#82fff64 check <skill-dir>
npx --yes github:rhdeck/monoskill#82fff64 check <skill-dir> --json
```
...
```bash
npx --yes github:rhdeck/monoskill#82fff64 update <skill-dir>
```
