...
The skill and CLI install separately. On a clean machine, use the exact public-registry invocation shown below; it downloads and runs Monoskill 0.3.2 without a prior global install. If `monoskill` 0.3.2 is already installed, `monoskill <command>` is equivalent. Run `npx --yes monoskill@0.3.2 --help` before acting.
...
```bash
npx --yes monoskill@0.3.2 add <source> --name <name> --dry-run --json
```

Install into the current project after inspecting the preview:

```bash
npx --yes monoskill@0.3.2 add <source> --name <name>
npx --yes monoskill@0.3.2 add <source> --name <name> --agent codex --agent claude-code
```
...
```bash
npx --yes monoskill@0.3.2 add <source> --name <name> --agent codex --agent claude-code --global --yes --json
```
...
```bash
npx --yes monoskill@0.3.2 build <source> --name <name> --output <dir>
```
...
```bash
npx --yes monoskill@0.3.2 build <source> --name <name> --archive --output <file.skill>
```
...
```bash
npx --yes monoskill@0.3.2 package <skill-dir> --output <file.skill>
```
...
```bash
npx --yes monoskill@0.3.2 check <skill-dir>
npx --yes monoskill@0.3.2 check <skill-dir> --json
```
...
```bash
npx --yes monoskill@0.3.2 update <skill-dir>
```
