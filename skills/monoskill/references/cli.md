...
The skill and CLI install separately. On a clean machine, use the exact public-registry invocation shown below; it downloads and runs Monoskill 0.4.0 without a prior global install. If `monoskill` 0.4.0 is already installed, `monoskill <command>` is equivalent. Run `npx --yes monoskill@0.4.0 --help` before acting.
...
`--name` is optional for `add` and `build`. For GitHub sources, the default combines owner and repository while removing a generic trailing `skills`: `mattpocock/skills` becomes `mattpocock`, and `coreyhaines31/marketingskills` becomes `coreyhaines31-marketing`. Keep `--name` when the user supplied an override or when an exact destination name is part of the request.
...
```bash
npx --yes monoskill@0.4.0 add <source> --name <name> --dry-run --json
```

Install into the current project after inspecting the preview:

```bash
npx --yes monoskill@0.4.0 add <source> --name <name>
npx --yes monoskill@0.4.0 add <source> --name <name> --agent codex --agent claude-code
```
...
```bash
npx --yes monoskill@0.4.0 add <source> --name <name> --agent codex --agent claude-code --global --yes --json
```
...
```bash
npx --yes monoskill@0.4.0 build <source> --name <name> --output <dir>
```
...
```bash
npx --yes monoskill@0.4.0 build <source> --name <name> --archive --output <file.skill>
```
...
```bash
npx --yes monoskill@0.4.0 package <skill-dir> --output <file.skill>
```
...
```bash
npx --yes monoskill@0.4.0 check <skill-dir>
npx --yes monoskill@0.4.0 check <skill-dir> --json
```
...
```bash
npx --yes monoskill@0.4.0 update <skill-dir>
```
